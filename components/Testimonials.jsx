...
export const AnimatedTestimonials = ({ testimonials, autoplay = false }) => {
  ...
  // Wait for rotations to be ready
  if (randomRotations.length === 0) return null;

  return (
    <div className="mx-auto max-w-sm px-4 py-20 my-40 font-sans antialiased md:max-w-6xl md:px-8 lg:px-12">
      <div className="relative grid grid-cols-1 gap-20 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-between py-4">
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-neutral-900">{testimonials[active].designation}</p>
          ...
          <div className="flex gap-4 pt-12 md:pt-0">
            <button onClick={handlePrev} className="group/button flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800">
              <IconArrowLeft className="h-9 w-9 text-white transition-transform duration-300 group-hover/button:rotate-12" />
            </button>
            <button onClick={handleNext} className="group/button flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-neutral-800">
